...
func (self *SCloudaccount) SyncDnsZones(ctx context.Context, userCred mcclient.TokenCredential, dnsZones []cloudprovider.ICloudDnsZone) ([]SDnsZone, []cloudprovider.ICloudDnsZone, compare.SyncResult) {
	...
	for i := 0; i < len(commondb); i += 1 {
		dnsZone, err := commondb[i].GetDnsZone()
		if err != nil {
			result.UpdateError(errors.Wrapf(err, "GetDnsZone for %s", commondb[i].Name))
			continue
		}

		if dnsZone.ZoneType == string(cloudprovider.PrivateZone) {
			err := dnsZone.syncWithCloudDnsZone(ctx, userCred, commonext[i], self.Id)
			if err != nil {
	...
	for i := 0; i < len(added); i += 1 {
		dnsZone, err := DnsZoneManager.newFromCloudDnsZone(ctx, userCred, added[i], self)
		if err != nil {
			result.AddError(err)
			continue
		}
