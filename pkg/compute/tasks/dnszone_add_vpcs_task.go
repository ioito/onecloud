...
func (self *DnsZoneAddVpcsTask) OnInit(ctx context.Context, obj db.IStandaloneModel, data jsonutils.JSONObject) {
	...
	iVpcMaps, accountId := map[string]cloudprovider.ICloudVpc{}, ""
	for _, vpcId := range vpcIds {
		...
		if len(iVpcMaps) > 0 && len(accountId) == 0 {
	...
	if len(accountId) > 0 {
		...
		if len(cache.ExternalId) > 0 {
			...
			for vpcId, iVpc := range iVpcMaps {
				...
				if err != nil {
					self.taskFailed(ctx, dnsZone, errors.Wrapf(err, "iDnsZone.AddVpc(%s)", opts.Id))
					return
				}
