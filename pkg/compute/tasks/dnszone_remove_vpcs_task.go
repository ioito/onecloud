...
func (self *DnsZoneRemoveVpcsTask) OnInit(ctx context.Context, obj db.IStandaloneModel, data jsonutils.JSONObject) {
	...
	vpcMaps, accountId := map[string]cloudprovider.ICloudVpc{}, ""
	...
	if len(accountId) > 0 {
		...
		if len(cache.ExternalId) > 0 {
			...
			for vpcId, iVpc := range vpcMaps {
				...
				if err != nil {
					self.taskFailed(ctx, dnsZone, errors.Wrapf(err, "iDnsZone.RemoveVpc(%s)", opts.Id))
					return
				}
