...
func (self *SHcsClient) request(method httputils.THttpMethod, url string, query url.Values, params map[string]interface{}) (jsonutils.JSONObject, error) {
	...
	var err error
	for i := 0; i < 4; i++ {
		_, resp, err = cli.Send(context.Background(), req, &hcsError{}, self.debug)
		if err == nil {
			break
		}
		if err != nil {
...
func (self *SHcsClient) _url(product, version, regionId string, resource string) string {
	...
	if strings.HasPrefix(resource, "images") || strings.HasPrefix(resource, "cloudimages") || strings.HasPrefix(resource, "nat_gateways") {
