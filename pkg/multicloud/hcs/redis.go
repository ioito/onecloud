...
func (self *SRegion) CreateIElasticcaches(opts *cloudprovider.SCloudElasticCacheInput) (cloudprovider.ICloudElasticcache, error) {
	zones, err := self.zoneNameToDcsZoneIds(opts.ZoneIds)
	if err != nil {
		return nil, err
	}
	...
	return ret, self.redisCreate("instances", params, ret)
}
...
func (self *SElasticcache) Delete() error {
	res := fmt.Sprintf("instances/%s", self.InstanceId)
	return self.region.redisDelete(res)
}
