...
func init() {
	...
	type HostedZoneCreateOptions struct {
		...
		Type   string `help:"PublicZone|PrivateZone"`
	...
	shellutils.R(&HostedZoneCreateOptions{}, "hostedzone-create", "Create hostedzone", func(cli *aws.SRegion, args *HostedZoneCreateOptions) error {
		...
		opts.Name = args.Name
		opts.Type = cloudprovider.TDnsZoneType(args.Type)
		opts.VpcId = args.Vpc
		opts.VpcRegionId = args.Region
		hostzones, err := cli.GetClient().CreateHostedZone(&opts)
		if err != nil {
			return err
		}
		printList(hostzones, 1, 0, 20, []string{})
		return nil
	})
	...
	shellutils.R(&HostedZoneAddVpcOptions{}, "hostedzone-addvpc", "associate vpc with hostedzone", func(cli *aws.SRegion, args *HostedZoneAddVpcOptions) error {
	...
	shellutils.R(&DnsRecordSetCreateOptions{}, "dnsrecordset-create", "create dnsrecordset", func(cli *aws.SRegion, args *DnsRecordSetCreateOptions) error {
		opts := cloudprovider.SDnsRecordSetChangeOptions{}
		opts.Name = args.Name
		opts.Type = args.Type
		opts.Value = args.Value
		opts.TTL = args.Ttl
		opts.Identify = args.Identify
